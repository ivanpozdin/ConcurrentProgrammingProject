...
import com.pseuco.cp25.model.Direction;
import com.pseuco.cp25.model.InfectionState;
...
public class Person {
    ...
    /**
     * Sets the infection state of the person and resets the
     * <em>inStateSince</em>-counter.
     *
    ...
    public void tick() {
        ...
        final XY newPosition = this.position.add(velocity);

        // check whether we would would bump into a wall
        if (!this.context.getGrid().contains(newPosition)) {
            this.direction = Direction.NONE;
            return;
        }

        // check whether we would bump into an obstacle
        if (this.context.getObstacles().stream().anyMatch(
                (Rectangle rectangle) -> rectangle.contains(newPosition)
        )) {
            this.direction = Direction.NONE;
            return;
        }
        ...
        if (this.context.getPopulation().stream().anyMatch(
                (Person person) -> person.getPosition().equals(newPosition)
                || (person.hasGhost()
                && person.getGhostPosition().equals(newPosition))
        )) {
            this.direction = Direction.NONE;
            return;
        }
